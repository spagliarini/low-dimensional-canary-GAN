...
def train(fps, args):
  ...
  with tf.name_scope('loader'):  #add scope as a prefix to all operations
    # x is a tuple of np.float32 tensors representing audio waveforms.
    # size audio: [batch_size, slice_len, 1, nch]
  ...
  # Make generator
  with tf.variable_scope('G'): #define the scope of a shared variable
    G_z = WaveGANGenerator(z, train=True, **args.wavegan_g_kwargs) #define generator
    #build the DCGAN generator architecture with the 5 convolution layer after having
    #projected and reshaped z (attention that here z is a matrix)
    #at the beginning of the training upsample ='zeros', so inside the function the first convolution
    #layer will be a transposed 2D convolution layer
    if args.wavegan_genr_pp:
      with tf.variable_scope('pp_filt'): #to use postprocess filter
        G_z = tf.layers.conv1d(G_z, 1, args.wavegan_genr_pp_len, use_bias=False, padding='same')
  G_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='G') #list of values in the collection of the first arg
  #which is all the variables created with trainable=True, and with the additional scope the resulting list is filtered to include
  #only items whose name attribute matches scope. Items without name never returned. Order of collection.
  ...
  # Summarize
  tf.summary.audio('x', x, args.data_sample_rate) #output a summary protocol buffer with audio
  ...
  if args.wavegan_loss == 'dcgan':
    ...
    G_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits( #mean of elements across dimensions of a tensor
      logits=D_G_z,
      labels=real
    ))
    ...
    #alpha is then the learning rate so it is optimized using AdamOptimizer later
    differences = G_z - x
    interpolates = x + (alpha * differences)
    with tf.name_scope('D_interp'), tf.variable_scope('D', reuse=True):
      D_interp = WaveGANDiscriminator(interpolates, **args.wavegan_d_kwargs)

    LAMBDA = 10 #original value = 10
  ...
  if args.wavegan_loss == 'dcgan':
    ...
    G_opt = tf.train.AdamOptimizer(
        ...
        beta1=0.5,
        beta2=0.9)
    D_opt = tf.train.AdamOptimizer(  #optimizer that implements the Adam algorithm, the values for the 
    #hyperparameters are the same used in the paper about Wavegan with gradient penalty (Gulrajani, 2017)
        learning_rate=1e-4,
        beta1=0.5,
        beta2=0.9)
  else:
  ...
  G_train_op = G_opt.minimize(G_loss, var_list=G_vars,
      global_step=tf.train.get_or_create_global_step()) #get or create global step in graph
...
"""
  Generates a preview audio file every time a checkpoint is saved (online, during learning)
"""
def preview(args):
  import matplotlib
  matplotlib.use('Agg')
...
"""
Generate the preview a posteriori.
This function is useful whenever you can't run the preview in parallel with 
the training.
"""
def preview_after(args):
    import matplotlib
    matplotlib.use('Agg')
    import matplotlib.pyplot as plt
    from scipy.io.wavfile import write as wavwrite
    ...
    preview_dir = os.path.join(args.train_dir, 'preview_after')
    if not os.path.isdir(preview_dir):
        os.makedirs(preview_dir)
    ...
    ckpt_list = glob.glob(args.train_dir + '/' + 'model.ckpt*data*')

    for i in range(0,np.size(ckpt_list)):
        # Choose the checkpoint
        sess = tf.InteractiveSession()
        ckpt_fp = os.path.join(ckpt_list[i][0:len(ckpt_list[i])-20])
        saver.restore(sess, ckpt_fp)
        _fetches = sess.run(fetches, feeds)
        _step = _fetches['step']

        # Generate the preview relative to that particular checkpoint
        preview_fp = os.path.join(preview_dir, '{}.wav'.format(str(_step).zfill(8)))
        wavwrite(preview_fp, args.data_sample_rate, _fetches['G_z_flat_int16'])
...
"""
Generator only. This function generates a posteriori generation_n sounds at checkpoint ckpt_n.
"""
def generation_S(args):
    import time
    ...
    # How many generated sound I want
    ngenerate = args.generation_n

    # Create the directory
    generation_dir = os.path.join(args.train_dir, 'generation_' + str(args.ckpt_n))
    #generation_dir = os.path.join(args.train_dir, 'Motor')
    ...
    if args.ckpt_n == False:
        ckpt_fp = tf.train.latest_checkpoint(args.train_dir)
    else:
        ckpt_fp = os.path.join(args.train_dir, 'model.ckpt-'+str(args.ckpt_n))
    saver.restore(sess, ckpt_fp)

    for i in range(0, ngenerate):
        # Sample latent vector
        _z = (np.random.rand(1, args.wavegan_latent_dim) * 2.) - 1
        generation_fp = os.path.join(generation_dir,'z'+str(i)+'.pkl')
        ...
        generation_fp = os.path.join(generation_dir, 'generation'+str(i) +'.wav')
        wavwrite(generation_fp, args.data_sample_rate, _G_z.T)

    songfile = glob.glob(generation_dir + '/' + '*.wav')
    for i in range(0, ngenerate):
        # read the syllable
        samples, sr = librosa.load(generation_dir + '/' + 'generation'+str(i) + '.wav', sr=16000)
        ...
        if np.size(samples) > 0:
            ...
            wavwrite(generation_dir + '/' + 'generation'+str(i) + '.wav', args.data_sample_rate, y)

        else:
            os.remove(generation_dir + '/' + 'generation'+str(i) + '.wav')

    print('Done')

"""
Generate one sound from a latent vector and then vary its component from -1 to 1 to see how
the generated sound evolves. 
"""
def variation(args):
    import pickle
    import numpy as np
    ...
    # Create the directory
    variation_dir = os.path.join(args.train_dir, 'variation_'  + str(args.variation_step)[-2::])
    ...
    generation_fp = os.path.join(variation_dir,'z_baseline.pkl')
    # Save z
    with open(generation_fp, 'wb') as f:
        pickle.dump(_z, f)
    ...
    generation_fp = os.path.join(variation_dir, 'generation_baseline.wav')
    wavwrite(generation_fp, args.data_sample_rate, _G_z.T)
    ...
    for i in range(0,args.wavegan_latent_dim):
        ...
        z_aux_pos = z_baseline.copy()
        z_aux_neg = z_baseline.copy()

        while (z_aux_pos[i] + args.variation_step) < 1:
            z_aux_pos[i] = z_aux_pos[i] + args.variation_step

            _z = np.reshape(z_aux_pos, (1, args.wavegan_latent_dim))

            generation_fp = os.path.join(variation_dir,'z_component_'+str(i)+'_pos'+str(counter_positive) + '.pkl')
            # Save z
            with open(generation_fp, 'wb') as f:
                pickle.dump(_z, f)
            ...
            generation_fp = os.path.join(variation_dir, 'gen_component_'+str(i)+'_pos'+str(counter_positive) +'.wav')
            wavwrite(generation_fp, args.data_sample_rate, _G_z.T)

            counter_positive = counter_positive + 1

        while (z_aux_neg[i] - args.variation_step) > -1:
            z_aux_neg[i] = z_aux_neg[i] - args.variation_step

            _z = np.reshape(z_aux_neg, (1, args.wavegan_latent_dim))

            generation_fp = os.path.join(variation_dir,'z_component_'+str(i)+'_neg'+str(counter_negative) + '.pkl')
            # Save z
            with open(generation_fp, 'wb') as f:
                pickle.dump(_z, f)
            ...
            generation_fp = os.path.join(variation_dir, 'gen_component_'+str(i)+'_neg'+str(counter_negative) +'.wav')
            wavwrite(generation_fp, args.data_sample_rate, _G_z.T)
...
if __name__ == '__main__':
  ...
  parser.add_argument('mode', type=str, choices=['train', 'infer', 'preview', 'preview_after',
                                                 'generation_S', 'variation', 'incept', 'get_loss'])
  ...
  wavegan_args.add_argument('--ckpt_n', type=int,
      help='At which chekpoint-it has to be saved')
  ...
  generation_S_args = parser.add_argument_group('Generation_S')
  generation_S_args.add_argument('--generation_n', type=int,
      help='Number of sounds to generate')
  generation_S_args.add_argument('--filter_freq', type=int, help='Frequency cut off for the filter, in Hz', default=700)
  generation_S_args.add_argument('--filter_order', type=int, help='Order of the filter', default=5)
  ...
  variation_args = parser.add_argument_group('Variation')
  ...
  parser.set_defaults(
    ...
    train_batch_size=64,
    train_save_secs= 1800,
    train_summary_secs= 1800,
    preview_n=20,
  ...
  if args.mode == 'train':
    ...
    infer(args)  #run once infer but then train (no savings in infer directory after the right beginning of the training)
    train(fps, args)
  elif args.mode == 'infer':
    infer(args)
  elif args.mode == 'preview':
    preview(args)
  elif args.mode == 'generation_S':
    generation_S(args)
  elif args.mode == 'preview_after':
    preview_after(args)
  elif args.mode == 'variation':
    variation(args)
  elif args.mode == 'incept':
    incept(args)
  elif args.mode == 'get_loss':
    get_loss(args)
  else:
    raise NotImplementedError()
