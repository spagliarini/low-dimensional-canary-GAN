...
def conv1d_transpose(
    ...
    upsample='zeros'):
  if upsample == 'zeros':
    return tf.layers.conv2d_transpose( #transposed 2D convolution layer
        tf.expand_dims(inputs, axis=1), #inserts a dimension of 1 into a tensor's shape
        filters,
        (1, kernel_width),
        strides=(1, stride),
        padding='same'
    ...
    return tf.layers.conv1d( #1D convolution layer
        x,
        filters,
        kernel_width,
        1,
...
def WaveGANGenerator(
    ...
    upsample='zeros',
    train=False):
  assert slice_len in [16384, 32768, 65536] #1,2 or 4 seconds
  ...
  output = z
  #then I update the output
  with tf.variable_scope('z_project'): #define the scope of a shared variable
    output = tf.layers.dense(output, 4 * 4 * dim * dim_mul) #Densely-connected layer class
    output = tf.reshape(output, [batch_size, 16, dim * dim_mul])
    #since my z is a matrix 64*100, then I will have 64 reshapes to 16*1024
    output = batchnorm(output)
  output = tf.nn.relu(output) #rectified linear max(features,0)
  dim_mul //= 2

  # Layer 0
  # [16, 1024] -> [64, 512]
  with tf.variable_scope('upconv_0'):
    output = conv1d_transpose(output, dim * dim_mul, kernel_len, 4, upsample=upsample) #1D or 2D convolution layer
    output = batchnorm(output)
  ...
  if slice_len == 16384:
    ...
    output = tf.nn.tanh(output) #element-wise hyperbolic tangent
...
#leaky rectified linear function max(features*X, X), where X is a small factor
#for the discriminator (necessary to solve the cases in which there is no gradient)
